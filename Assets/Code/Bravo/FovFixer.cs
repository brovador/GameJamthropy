...
public class FovFixer : MonoBehaviour {
	...
	public float originalAspectRatio = 1024.0f / 768.0f;
	...
	public void OnDestroy() {
		if(OrientationManager.Instanced())
			OrientationManager.Instance.OnOrientationChanged -= OnOrientationChanged;
	}
	...
	float Aspect() {
		if(cam.enabled) {
			...
			return (Screen.width * cam.rect.width) / (Screen.height * cam.rect.height);
		}
	}
	...
	protected virtual void Fit() {
		...
		float aspectRatio = previousAspect;
